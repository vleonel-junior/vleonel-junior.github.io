...
## Introduction : le pipeline de préparation des données
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.1 : Les trois grandes étapes pour coder un LLM. Ce chapitre se concentre sur l'étape 1 de la phase 1 : l'implémentation du pipeline d'échantillonnage de données. Source : Raschka (2024).*</figcaption>
</figure>
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.4 : Une vue des étapes de traitement du texte dans le contexte d'un LLM. Ici, nous divisons un texte d'entrée en tokens individuels (mots ou caractères spéciaux). Source : Raschka (2024).*</figcaption>
</figure>
...
### Un tokenizer simple avec les expressions régulières
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.5 : Le schéma de tokenization sépare correctement le texte en mots individuels et ponctuations. Source : Raschka (2024).*</figcaption>
</figure>
...
## Convertir les tokens en identifiants
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.6 : Construction d'un vocabulaire en tokenisant l'ensemble du dataset d'entraînement. Les tokens sont extraits, triés par ordre alphabétique, et les doublons retirés. Le vocabulaire fait correspondre chaque token unique à une valeur entière unique. Source : Raschka (2024).*</figcaption>
</figure>
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.7 : À partir d'un nouvel échantillon de texte, nous le tokenisons et utilisons le vocabulaire pour convertir les tokens textuels en token IDs. Source : Raschka (2024).*</figcaption>
</figure>


### Implémenter une classe Tokenizer
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.8 : Les implémentations de tokenizers partagent deux méthodes communes : encode (convertit le texte en ID via le vocabulaire) et decode (reconvertit les IDs en texte naturel). Source : Raschka (2024).*</figcaption>
</figure>
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.9 : Ajout des tokens spéciaux `<|unk|>` (pour les mots inconnus) et `<|endoftext|>` (pour séparer deux sources de texte non liées) au vocabulaire. Source : Raschka (2024).*</figcaption>
</figure>
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.10 : Lors du traitement de plusieurs sources de texte indépendantes, le token `<|endoftext|>` agit comme un marqueur signalant le début ou la fin d'un segment. Source : Raschka (2024).*</figcaption>
</figure>
...
## L'encodage par paire d'octets (BPE)
...
Ici, nous allons utiliser une bibliothèque open-source Python existante appelée `tiktoken` (https://github.com/openai/tiktoken), qui implémente l'algorithme BPE de manière très efficace. De même que pour d'autres bibliothèques Python, nous pouvons installer la bibliothèque `tiktoken` via l'installateur de paquets `pip` depuis le terminal :
...
>
> *Figure 2.11 : Les tokenizers BPE décomposent les mots inconnus en sous-mots et en caractères individuels. Ainsi, un tokenizer BPE peut analyser n'importe quel mot et n'a pas besoin de remplacer les mots inconnus par des tokens spéciaux, tels que `<|unk|>`. Source : Raschka (2024).*
...
## Échantillonner les données avec une fenêtre glissante
...
>
> *Figure 2.12 : Extraction de blocs d'entrée (input blocks) à partir d'un échantillon de texte pour l'entraînement du LLM. La tâche consiste à prédire le mot suivant le bloc d'entrée, en masquant les mots qui suivent la cible. (La tokenisation est omise ici pour plus de clarté). Source : Raschka (2024).*
...
>
> *Figure 2.13 : Pour une efficacité maximale, les entrées sont regroupées dans un tenseur `x`, où chaque ligne représente le contexte d'entrée. Un second tenseur `y` contient les cibles prédictives correspondantes (les mots suivants), créées en décalant l'entrée d'une position. Source : Raschka (2024).*
...
>
> *Figure 2.14 : En définissant un pas (stride) égal à la taille de la fenêtre d'entrée (input window size), on évite tout chevauchement entre les lots. Source : Raschka (2024).*
...
## Créer les embeddings de tokens
...
>
> *Figure 2.15 : La préparation du texte passe par la tokenisation, la conversion en identifiants (token IDs), et enfin la projection de ces identifiants en vecteurs continus via une couche d'embedding. Source : Raschka (2024).*

</div>

### Fonctionnement d'une couche d'embedding
...
>
> *Figure 2.16 : L'extraction de vecteurs d'embedding. Chaque token ID sert d'index pour extraire la ligne correspondante depuis la matrice de poids de la couche d'embedding. Source : Raschka (2024).*
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.17 : La couche d'embedding assigne la même représentation vectorielle à un token,
...
d'embedding. Source : Raschka (2024).*</figcaption>
</figure>
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.18 : Les embeddings positionnels sont additionnés aux embeddings de tokens pour
former les embeddings d'entrée finaux du LLM. Les vecteurs positionnels ont la même dimension
que les embeddings de tokens. Source : Raschka (2024).*</figcaption>
</figure>
...
Nous travaillons ici avec une dimension d'embedding de 256 (inférieure aux 12 288 dimensions de GPT-3, mais suffisante pour l'expérimentation) et le tokenizer BPE introduit précédemment, qui couvre un vocabulaire de 50 257 tokens.
...
<figcaption class="text-sm text-center text-gray-400 italic">*Figure 2.19 : Résumé du pipeline de traitement de l'entrée. Le texte est tokenisé, converti en `token_ids`, transformé en embeddings de tokens, auxquels on additionne les embeddings positionnels pour former les `input_embeddings` transmis aux couches principales du LLM. Source : Raschka (2024).*</figcaption>
</figure>
