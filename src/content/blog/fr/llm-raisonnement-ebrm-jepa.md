...
Il y a trois causes distinctes à ce blocage, et il vaut la peine de les séparer parce qu'elles ne se résolvent pas de la même manière ni avec les mêmes outils. Cette décomposition en trois points reprend et structure les idées que Yann LeCun développe depuis plusieurs années concernant les limites des approches actuelles de l'IA [1, 2].

**a) La génération autorégressive rend la révision coûteuse.**
Un LLM produit sa réponse token après token, de gauche à droite, chaque nouveau mot étant conditionné sur tous ceux qui précèdent. C'est un peu comme écrire une lettre avec un stylo, sans gomme ni correcteur : une fois qu'une phrase est posée, revenir dessus pour la corriger oblige à raturer et à réécrire tout ce qui suit, puisque la suite du texte a été pensée en fonction de ce qui était déjà là. Formellement, on dit que le modèle a du mal à faire du conditionnement "à l'envers", c'est-à-dire produire un raisonnement en tenant compte à la fois du point de départ et d'un objectif final donné à l'avance. Cela rend difficile l'attribution de crédit : quand un raisonnement échoue, il est difficile d'identifier précisément quelle étape antérieure en est la cause, et donc de la corriger sans tout reprendre.
...
LeCun rattache d'ailleurs cette approche à la théorie classique de la résolution de problèmes en intelligence artificielle [1] : un système de recherche efficace a toujours besoin de deux composants distincts, un qui propose des solutions candidates et un autre qui évalue si elles sont bonnes *avant* que la solution soit terminée. Sans cela, il n'y a plus de recherche véritable, seulement des tentatives indépendantes jugées après coup, exactement la « recherche par tâtonnement » dont il vient d'être question. Sa critique des LLM porte précisément là-dessus : produire plusieurs suites de tokens et les noter une fois qu'elles sont complètes n'est, selon lui, qu'une forme très primitive de cette recherche, qui devrait idéalement s'exécuter dans un espace de représentation continu plutôt que dans l'espace du langage lui-même [1].
...
https://logicalintelligence.com/blog/energy-based-model-sudoku-demo (démonstration interactive : https://sudoku.logicalintelligence.com/)
*(Billet d'entreprise avec démonstration publique en temps réel, limitée à un problème borné et de petite taille, sans audit tiers indépendant du chiffre.)*
