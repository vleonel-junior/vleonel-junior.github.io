...
**Adding two-digit numbers.** The predicate becomes $\mathrm{addition}([\mathrm{img}(X_1), \mathrm{img}(X_2)], [\mathrm{img}(Y_1), \mathrm{img}(Y_2)], n)$, where each list represents a two-digit number. $\mathrm{addition}([\mathrm{img}(2), \mathrm{img}(0)], [\mathrm{img}(1), \mathrm{img}(7)], 37)$ is valid, $\mathrm{addition}([\mathrm{img}(5), \mathrm{img}(4)], [\mathrm{img}(9), \mathrm{img}(0)], 50)$ is not.
