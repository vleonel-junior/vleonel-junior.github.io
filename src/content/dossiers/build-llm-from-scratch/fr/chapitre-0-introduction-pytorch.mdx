...
### Créer des tenseurs et choisir leur type
...
### Forme et remodelage : `.reshape()` et `.view()`
...
#### Comment un tenseur devient non contigu
...
#### Pourquoi `.reshape()` fonctionne toujours
...
### Transposer et multiplier des matrices
...
### Le forward pass d'une régression logistique
...
<div align="center">
  ...
  <p><em>Forward pass d'une régression logistique représenté comme un graphe de calcul. La caractéristique d'entrée x1 est multipliée par un poids du modèle w1 et passée à travers une fonction d'activation σ après avoir ajouté le biais. La loss est calculée en comparant la sortie du modèle a avec un label donné y. Source : Raschka (2024).</em></p>
</div>
...
### Calculer des gradients avec `grad`
...
<div align="center">
  ...
  <p><em>Un perceptron multicouche avec deux couches cachées. Chaque nœud représente une unité dans la couche respective. À des fins d'illustration, chaque couche a un très petit nombre de nœuds. Source : Raschka (2024).</em></p>
</div>
...
### Rendre l'initialisation reproductible
...
### La passe forward et l'inférence
...
<div align="center">
  ...
  <p><em>PyTorch implémente une classe Dataset et une classe DataLoader. La classe Dataset est utilisée pour instancier des objets qui définissent comment chaque enregistrement de données est chargé. Le DataLoader gère la manière dont les données sont mélangées et assemblées en batchs. Source : Raschka (2024).</em></p>
</div>

### Un jeu de données jouet avec `Dataset` et `DataLoader`
...
### Sous le capot : le DataLoader et ses workers
...
---
### Pourquoi passer les logits directement à la cross-entropy ?
...
### Faire des prédictions et mesurer la précision
...
### Exercice A.4 : le CPU ou le GPU, lequel est le plus rapide ?
...
### Entraînement multi-GPU : stratégies et mécanismes
...
`torch.nn.DataParallel` est l'API historique de PyTorch pour l'entraînement multi-GPU. Pour bien comprendre ses limites, il faut d'abord comprendre comment elle fonctionne de l'intérieur, ce qui commence par deux notions fondamentales : ce qu'est un **processus** et ce qu'est un **thread**.
...
<div align="center">
  ...
  <p><em>Le transfert du modèle et des données dans DDP implique deux étapes clés. Tout d'abord, nous créons une copie du modèle sur chacun des GPU. 
         Ensuite, nous divisons les données d'entrée en minibatches uniques que nous transmettons à chaque copie du modèle. Source : Raschka (2024).</em></p>
</div>
...
<div align="center">
  ...
  Cela garantit que chaque réplique du modèle possède les mêmes poids mis à jour. Source : Raschka (2024).</em></p>
</div>
...
Le processus 1 lit cette condition, constate qu'il n'est pas le rang 0, et ne produit aucune sortie. Seul le processus 0 s'exprime, et les duplications disparaissent.
...
* **Les tenseurs** sont la structure de données centrale de PyTorch. `.reshape()` fonctionne dans tous les cas, alors que `.view()` exige un tenseur contigu en mémoire, ce qui n'est plus le cas après une transposition.
* **Autograd** construit un graphe de calcul pendant la passe forward et applique la règle de dérivation en chaîne pour obtenir les gradients, soit à la demande avec `grad`, soit pour tous les paramètres avec `.backward()`.
* **Un réseau de neurones** se définit en héritant de `torch.nn.Module` : les couches dans `__init__`, le flux des données dans `forward`. Les modèles renvoient des logits, et la softmax est appliquée par la fonction de perte.
* **`Dataset` et `DataLoader`** séparent la lecture des exemples de leur assemblage en batchs. Les workers, la *shared memory* et `pin_memory` préparent les batchs suivants pendant que le GPU calcule.
* **La boucle d'entraînement** enchaîne passe forward, calcul de la loss, `zero_grad()`, `backward()` et `step()`. `model.train()` et `model.eval()` basculent le comportement des couches comme le dropout.
* **Sauvegarder un modèle** revient à enregistrer son `state_dict`, puis à le recharger dans une instance de la même architecture.
* **Sur GPU**, le modèle et les données doivent être sur le même device. Au-delà d'un GPU, DDP réplique le modèle et synchronise les gradients avec AllReduce, tandis que le parallélisme de modèle répartit le modèle lui-même quand il ne tient plus dans un seul GPU.
